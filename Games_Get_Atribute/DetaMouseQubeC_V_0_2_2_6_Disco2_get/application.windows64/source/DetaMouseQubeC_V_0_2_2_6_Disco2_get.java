...
public class DetaMouseQubeC_V_0_2_2_6_Disco2_get extends PApplet {
...
movet=1;
  ...
  if(movet==1){
   ...
   rdmp=(int)random(-3,3);
     rdmps=rdmps+v5/5+ (int)random(-score2,score2/3)/3;
...
if(enemyy<=20){enemyy3++;enemyy3++;rdmps1=0;}
 ...
 if((enemyy2)>=300){enemyy2--;enemyy2--;rdmps3=0;}
    ...
    //rect(abs(enemyx+v1+v6),abs(enemyy+v3+v6) ,abs(enemyx2+v4+v6), abs(enemyy2+v5+v6)); 
  ...
  while ( i < (b*(v2/3)+v4)) { 
    ...
    shipm=color( zzz1,xxx1,yyy1,170);
       ...
       int zint=(int)sqrt(v1+v2+v3+v4+v5+v6); 
      i++;  
        winkel = random(3*PI);    
        laenge = random(min(abs(width/2-v4), height/(zint*2)));   
        dx = cos(winkel)*laenge+v3;   
        dy = sin(winkel)* laenge+v3; 
          dz = tan(winkel)* laenge+v3; 
...
sap=0;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "DetaMouseQubeC_V_0_2_2_6_Disco2_get" };
    if (passedArgs != null) {
